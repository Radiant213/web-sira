...
@section('title', 'Verifikasi Perubahan Profil')

@section('content')
    <h2 class="mb-6 text-center text-xl font-bold text-white">Verifikasi Perubahan</h2>
    ...
    <form action="{{ route('profile.verify') }}" method="POST" class="space-y-5">
        @csrf
    ...
    <div class="mt-8 text-center text-sm text-slate-400">
        ...
        <form action="{{ route('profile.verify.resend') }}" method="POST" class="inline">
            @csrf
