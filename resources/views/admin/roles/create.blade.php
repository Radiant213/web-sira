...
@section('page-subtitle', 'Buat peran baru untuk sistem')
...
<div class="mx-auto max-w-2xl animate-fade-in-up">
    <div class="mb-6 flex items-center gap-4">
        ...
        <h2 class="text-xl font-bold text-slate-800">Tambah Role</h2>
    </div>

    <div class="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
        <form action="{{ route('admin.roles.store') }}" method="POST" class="p-6 sm:p-8">
            @csrf
            
            <div class="mb-6">
                <label for="name" class="mb-2 block text-sm font-semibold text-slate-700">Nama Role</label>
                <input type="text" name="name" id="name" value="{{ old('name') }}" class="block w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm transition-colors focus:border-primary-500 focus:bg-white focus:ring-primary-500 @error('name') border-rose-500 focus:border-rose-500 focus:ring-rose-500 @enderror" placeholder="Misal: Sekretaris" required>
                @error('name')
                    <p class="mt-2 text-sm text-rose-500">{{ $message }}</p>
                @enderror
                <p class="mt-2 text-xs text-slate-500">Nama role sebaiknya menggunakan huruf kecil tanpa spasi khusus jika akan digunakan dalam kode.</p>
            </div>
