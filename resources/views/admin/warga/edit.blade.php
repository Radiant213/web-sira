...
@section('page-subtitle', 'Perbarui informasi data warga')
...
<div class="mx-auto max-w-2xl animate-fade-in-up">
    <div class="rounded-2xl border border-border bg-card p-6 shadow-sm sm:p-8">
        ...
        <form method="POST" action="{{ route('admin.warga.update', $warga) }}" class="space-y-5">
            @csrf @method('PUT')

            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div class="sm:col-span-2">
                    <label for="nik" class="mb-1.5 block text-sm font-medium text-slate-700">NIK</label>
                    <input type="text" id="nik" name="nik" value="{{ old('nik', $warga->nik) }}" required maxlength="16"
                ...
                <div>
                    ...
                    <input type="text" id="phone" name="phone" value="{{ old('phone', $warga->phone) }}"
                           class="w-full rounded-xl border border-border px-4 py-3 text-sm outline-none transition-all focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20">
                </div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="address" name="address" rows="3"
                              class="w-full rounded-xl border border-border px-4 py-3 text-sm outline-none transition-all focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 resize-none">{{ old('address', $warga->address) }}</textarea>
                </div>
            </div>
